...
describe("batch_flag_emails", () => {
  ...
  describe("Tool handler", () => {
    it("should register and execute batch_move_emails tool", async () => {
      const { McpServer } = await import("@modelcontextprotocol/sdk/server/mcp.js");
      const { registerBatchMailTools } = await import("../src/tools/batch-mail.js");
      ...
      testServer.tool = (name: string, description: string, schema: unknown, handler: unknown) => {
        if (name === "batch_move_emails") {
          capturedHandler = handler as (params: unknown) => Promise<CallToolResult>;
        }
        return originalTool(name, description, schema, handler);
      };

      registerBatchMailTools(testServer, graphClient, testConfig);
    ...
    it("should register without throwing", async () => {
      ...
      expect(() => registerBatchMailTools(testServer, graphClient, testConfig)).not.toThrow();
    });
  });
});
